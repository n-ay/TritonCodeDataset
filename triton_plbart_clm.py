...
# kernel path: /tmp/torchinductor_root/tr/ctrql5mjvifq6a7y5hba33agwr4d74f5efh2ij5pgqjbn6aaltsy.py
...
#   hidden_states_5 => add_6, add_7, mul_5, mul_6, rsqrt_1, sub_2, var_mean_1
# Graph fragment:
...
#   %add_7 : [num_users=2] = call_function[target=torch.ops.aten.add.Tensor](args = (%mul_6, %arg15_1), kwargs = {})
triton_per_fused_add_native_layer_norm_2 = async_compile.triton('triton_per_fused_add_native_layer_norm_2', '''
import triton
import triton.language as tl
...
# Source node to ATen node mapping:
#   hidden_states_6 => add_8, erf, mul_7, mul_8, mul_9
# Graph fragment:
...
#   %mul_9 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%mul_7, %add_8), kwargs = {})
triton_poi_fused_gelu_3 = async_compile.triton('triton_poi_fused_gelu_3', '''
import triton
import triton.language as tl
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        # Topologically Sorted Source Nodes: [embedding, inputs_embeds, add_1, positions, hidden_states, hidden_states_1], Original ATen: [aten.embedding, aten.mul, aten.add, aten.native_layer_norm]
        ...
        # Topologically Sorted Source Nodes: [padding_mask, padding_mask_1, masked_fill, eq_1, all_1, attn_output], Original ATen: [aten.add, aten.eq, aten.masked_fill, aten.all, aten._scaled_dot_product_efficient_attention]
        stream0 = get_raw_stream(0)
        triton_per_fused__scaled_dot_product_efficient_attention_add_all_eq_masked_fill_1.run(arg2_1, buf8, 32, 32, stream=stream0)
        del arg2_1
        # Topologically Sorted Source Nodes: [attn_output], Original ATen: [aten._scaled_dot_product_efficient_attention]
        buf9 = torch.ops.aten._scaled_dot_product_efficient_attention.default(reinterpret_tensor(buf4, (1, 2, 32, 32), (2048, 32, 64, 1), 0), reinterpret_tensor(buf5, (1, 2, 32, 32), (2048, 32, 64, 1), 0), reinterpret_tensor(buf6, (1, 2, 32, 32), (2048, 32, 64, 1), 0), reinterpret_tensor(buf8, (1, 2, 32, 32), (1024, 0, 32, 1), 0), False, scale=0.1767766952966369)
        del buf8
        buf10 = buf9[0]
        ...
        # Topologically Sorted Source Nodes: [hidden_states_4, hidden_states_5], Original ATen: [aten.add, aten.native_layer_norm]
        stream0 = get_raw_stream(0)
        triton_per_fused_add_native_layer_norm_2.run(buf18, buf14, arg13_1, arg14_1, arg15_1, 32, 64, stream=stream0)
        del arg13_1
        del arg14_1
        del arg15_1
        buf19 = empty_strided_cuda((32, 128), (128, 1), torch.float32)
        # Topologically Sorted Source Nodes: [], Original ATen: [aten.addmm]
        extern_kernels.mm(reinterpret_tensor(buf18, (32, 64), (64, 1), 0), reinterpret_tensor(arg16_1, (64, 128), (1, 64), 0), out=buf19)
        del arg16_1
        buf20 = reinterpret_tensor(buf19, (1, 32, 128), (4096, 128, 1), 0); del buf19  # reuse
        # Topologically Sorted Source Nodes: [hidden_states_6], Original ATen: [aten.gelu]
        stream0 = get_raw_stream(0)
        triton_poi_fused_gelu_3.run(buf20, arg17_1, 4096, stream=stream0)
        del arg17_1
        buf21 = buf14; del buf14  # reuse
        # Topologically Sorted Source Nodes: [], Original ATen: [aten.addmm]
        extern_kernels.mm(reinterpret_tensor(buf20, (32, 128), (128, 1), 0), reinterpret_tensor(arg18_1, (128, 64), (1, 128), 0), out=buf21)
        del arg18_1
        del buf20
        ...
        stream0 = get_raw_stream(0)
        triton_per_fused_add_native_layer_norm_2.run(buf25, buf21, arg19_1, arg20_1, arg21_1, 32, 64, stream=stream0)
        del arg19_1
        del arg20_1
        del arg21_1
        del buf21
    return (reinterpret_tensor(buf6, (1, 2, 32, 32), (2048, 32, 64, 1), 0), reinterpret_tensor(buf5, (1, 2, 32, 32), (2048, 32, 64, 1), 0), buf25, )
